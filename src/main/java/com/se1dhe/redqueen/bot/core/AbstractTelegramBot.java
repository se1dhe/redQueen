...
package com.se1dhe.redqueen.bot.core;

import com.se1dhe.redqueen.bot.core.handlers.*;
import com.se1dhe.redqueen.bot.core.util.BotUtil;
import com.se1dhe.redqueen.bot.core.util.IThrowableFunction;
        ...
        try {
            ...
            for (IUpdateHandler updateHandler : updateHandlers) {
                try {
                    ...
                    log.error("Uncaught exception in onUpdate: {}", update, ex);
                }
            }
            ...
            if (update.getMessage().isSuperGroupMessage()) {
                handleUpdate(IGroupMessageHandler.class, update, Update::getMessage, Message::getFrom, handler -> handler.onGroupMessage(this, update, update.getMessage()));
                return;
            }
            ...
            if (unknownHandlers.isEmpty()) {
                log.warn("Update doesn't contains neither ChosenInlineQuery/InlineQuery/CallbackQuery/EditedMessage/ChannelPost/EditedChannelPost/Message Update: {}", update);
                return;
            }
            ...
            for (IUnknownUpdateHandler unknownHandler : unknownHandlers) {
                try {
                    ...
                    log.error("Uncaught exception in onUnhandledUpdate: {}", update, ex);
                }
            }
    ...
    private <T extends ITelegramHandler, R> void handleUpdate(Class<T> clazz, Update update, Function<Update, R> dataMapper, Function<R, User> idMapper, IThrowableFunction<T, Boolean> action) {
        ...
        for (T handler : handlers) {
            try {
                ...
                log.warn("Exception caught on handler: {} error: {}", handler.getClass().getSimpleName(), e.getApiResponse(), e);
            } catch (Exception e) {
                log.warn("Exception caught on handler: {}", handler.getClass().getSimpleName(), e);
            }
        }
    }
        ...
        if (matcher.find()) {
            ...
            if (handler != null) {
                try {
                    ...
                    log.warn("API Exception caught on handler: {}, response: {} message: {}", handler.getClass().getSimpleName(), e.getApiResponse(), message, e);
                } catch (Exception e) {
                    log.warn("Exception caught on handler: {}, message: {}", handler.getClass().getSimpleName(), message, e);
                }
            } else {
                for (IMessageHandler messageHandler : getAvailableHandlersForUser(IMessageHandler.class, message.getFrom())) {
                    try {
                        ...
                        log.warn("API Exception caught on handler: {}, response: {} message: {}", messageHandler.getClass().getSimpleName(), e.getApiResponse(), message, e);
                    } catch (Exception e) {
                        log.warn("Exception caught on handler: {}, message: {}", messageHandler.getClass().getSimpleName(), message, e);
                    }
                }
            }
        }
